...
function calculate(next){
   ...
   fetch('https://shopee.ph/api/v4/order/get_order_list?limit=5&list_type=3&offset='+next, opts).then(function (response) {
      ...
      .then(function (body) {
         ...
         if(next_offset >= 0){
            for (let [key, value] of Object.entries(body.data.details_list)) {
               ...
               total += total_temp;
               order++;
               console.log(order + ":", "PHP " + total_temp + " - ", value.info_card.order_list_cards[0].items[0].name);
            }
